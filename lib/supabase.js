...
export const assignIssueToDepartment = async (issueId, departmentId, notes = '') => {
  try {
    ...
    // Get department admin for the selected department
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        assigned_department_id: departmentId,
        current_assignee_id: departmentAdmin?.id || null,
...
// Enhanced tender functions with workflow integration
export const assignTenderToContractor = async (tenderId, contractorId, notes = '') => {
  try {
    const { user } = await getCurrentUser();
    if (!user) throw new Error('User not authenticated');
    ...
    // Create assignment record if there's a source issue
    if (tender.source_issue_id) {
      const { error: assignmentError } = await supabase
        .from('issue_assignments')
        .insert([{
          issue_id: tender.source_issue_id,
          assigned_by: user.id,
          assigned_to: contractorId,
          assignment_type: 'department_to_contractor',
          assignment_notes: notes,
          status: 'active'
        }]);
      ...
      const { error: issueError } = await supabase
        .from('issues')
        .update({
          current_assignee_id: contractorId,
    ...
    console.error('Assign tender to contractor error:', error);
    return { error };
  }
};
...
export const getWorkProgress = async (contractorId = null, issueId = null, tenderId = null) => {
  try {
    let query = supabase
      .from('work_progress')
      .select(`
...
// Enhanced dashboard functions with proper workflow integration
export const getAreaSuperAdminDashboard = async () => {
  try {
    const { user } = await getCurrentUser();
    if (!user) throw new Error('User not authenticated');
    ...
    // Get issues for the assigned area with enhanced data
    const { data: issues, error: issuesError } = await supabase
      .from('issues')
      .select(`
        *,
      ...
      `)
      .eq('assigned_area_id', profile.assigned_area_id)
      .order('created_at', { ascending: false });
...
export const getDepartmentAdminDashboard = async () => {
  try {
    ...
    // Get issues assigned to this department with enhanced data
    const { data: issues, error: issuesError } = await supabase
      .from('issues')
      .select(`
        *,
    ...
    // Get tenders created by this department with enhanced data
    const { data: tenders, error: tendersError } = await supabase
      .from('tenders')
      .select(`
        *,
...
export const getContractorDashboard = async () => {
  try {
    ...
    const { data: profile } = await getUserProfile(user.id);
    if (!profile || profile.user_type !== 'tender') {
      throw new Error('Access denied');
    }

    // Get available tenders
    const { data: availableTenders, error: tendersError } = await supabase
      .from('tenders')
      .select(`
        *,
        posted_by_profile:posted_by (
      ...
      `)
      .in('status', ['available', 'bidding_closed', 'under_review'])
      .order('created_at', { ascending: false });
    ...
    // Get assigned work (awarded tenders)
    const { data: assignedWork, error: workError } = await supabase
      .from('tenders')
      .select(`
        *,
        source_issue:source_issue_id (
...
export const submitWorkProgress = async (progressData) => {
  try {
    const { user } = await getCurrentUser();
    if (!user) throw new Error('User not authenticated');
    ...
    // If work is completed, update issue status
    if (progressData.status === 'completed' && progressData.issue_id) {
      const { error: issueError } = await supabase
        .from('issues')
        .update({
          status: 'resolved',
    ...
    console.error('Submit work progress error:', error);
    return { data: null, error };
  }
};

// Enhanced workflow stage functions
...
// Notification functions for workflow
...
export const evaluateBid = async (evaluationData) => {
  try {
    const { user } = await getCurrentUser();
    if (!user) throw new Error('User not authenticated');
